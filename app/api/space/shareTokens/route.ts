...
export async function GET(req: NextRequest) {
  try {
    const board = getApiParam(req, "board", true);
...
export async function POST(req: NextRequest) {
  ...
  const expiresAt = getApiParam(req, "expiresAt", true);
  const readOnly = getApiParam(req, "readOnly", true);
  const board = getApiParam(req, "board", false);
